...
export class InventoryPlusService {
  ...
  async findAll(page = 1, limit = 20): Promise<{ items: Inventory[]; total: number }> {
  ...
  async findLowStock(page = 1, limit = 20): Promise<{ items: Inventory[]; total: number }> {
  ...
  async findOutOfStock(page = 1, limit = 20): Promise<{ items: Inventory[]; total: number }> {
    const [items, total] = await this.inventoryRepository.findAndCount({
  ...
  async getAlerts(page = 1, limit = 20): Promise<{ items: StockAlert[]; total: number }> {
  ...
  async getMovements(page = 1, limit = 20): Promise<{ items: InventoryAudit[]; total: number }> {
  ...
  async adjustStock(dto: AdjustStockDto): Promise<Inventory> {
    const variant = await this.variantRepository.findOne({ where: { id: dto.variantId } });
    if (!variant) {
      throw new NotFoundException('Variant not found');
    }

    let inventory = await this.inventoryRepository.findOne({ where: { variantId: dto.variantId } });
    if (!inventory) {
  ...
  async checkAndCreateAlerts(): Promise<number> {
    ...
    for (const inv of inventories) {
      const threshold = inv.reorderPoint > 0 ? inv.reorderPoint : inv.lowStockThreshold;
      ...
      if (inv.availableQuantity <= 0) {
        const existingAlert = await this.alertRepository.findOne({
          where: { variantId: inv.variantId, isResolved: false, alertType: 'OUT_OF_STOCK' },
        });
        if (!existingAlert) {
          await this.alertRepository.save(this.alertRepository.create({
            variantId: inv.variantId,
  ...
  async resolveAlerts(): Promise<number> {
    const alerts = await this.alertRepository.find({ where: { isResolved: false } });
    let resolvedCount = 0;

    for (const alert of alerts) {
      const inv = await this.inventoryRepository.findOne({ where: { variantId: alert.variantId } });
      if (inv) {
        const threshold = inv.reorderPoint > 0 ? inv.reorderPoint : inv.lowStockThreshold;
