...
export class AdminFinancialReportsController {
  ...
  async getProfitLoss(@Query('dateFrom') dateFrom?: string, @Query('dateTo') dateTo?: string) {
    return this.reportService.getProfitLoss(dateFrom, dateTo);
  }

  @Get('revenue')
  @Permissions(DefaultPermissions.FINANCE_VIEW)
  async getRevenue(@Query('dateFrom') dateFrom?: string, @Query('dateTo') dateTo?: string) {
    return this.reportService.getRevenueReport(dateFrom, dateTo);
  }

  @Get('expenses')
  @Permissions(DefaultPermissions.FINANCE_VIEW)
  async getExpenses(@Query('dateFrom') dateFrom?: string, @Query('dateTo') dateTo?: string) {
    return this.reportService.getExpenseReport(dateFrom, dateTo);
  }

  @Get('settlements')
  @Permissions(DefaultPermissions.FINANCE_VIEW)
  async getSettlements(@Query('dateFrom') dateFrom?: string, @Query('dateTo') dateTo?: string) {
